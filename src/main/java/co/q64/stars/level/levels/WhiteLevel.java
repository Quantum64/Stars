...
public class WhiteLevel implements Level {
    ...
    protected @Inject @Getter WhiteFormingBlockType symbolicBlock;

    private final @Getter LevelType type = LevelType.WHITE;

    protected @Inject WhiteLevel() {}

    public BlockPos createChallenge(ServerWorld world, BlockPos start) {
        return structures.get(StructureType.CHALLENGE_WHITE).placeChallenge(world, start);
    }
}
