...
public class TrophyBlockRender extends TileEntityRenderer<TrophyTile> {
    ...
    public void render(TrophyTile tile, double x, double y, double z, float partialTicks, int destroyStage) {
        ...
        Minecraft.getInstance().getBlockRendererDispatcher().getBlockModelRenderer().renderModel(Minecraft.getInstance().world, model, trophyBlock.getDefaultState(), BlockPos.ZERO, buffer, false, random, trophyBlock.getDefaultState().getPositionRandom(new BlockPos(x, y, z)), EmptyModelData.INSTANCE);
        buffer.setTranslation(0, 0, 0);
        tessellator.draw();
